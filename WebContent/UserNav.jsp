...
<div class="w3-top">
 <ul class="w3-navbar w3-theme-d2 w3-left-align w3-large">
  ...
  <li class="w3-hide-small w3-dropdown-hover">
   <c:set var="count" value='<%=new UserDAO().getCountForTable("announcement")%>' />
    ...
    <div class="w3-dropdown-content w3-white w3-card-4">
      <sql:query var="rs" dataSource="${dataSource}">
							SELECT announcement FROM announcement ORDER BY anid DESC LIMIT 2
		</sql:query>

    <div class="w3-dropdown-content w3-white w3-card-4">
    <c:if test="${rs!=null}">
			<c:forEach var="row" items="${rs.rows}">
					<td><a href="userNotification.jsp">"${row.announcement}"</a></td>
			</c:forEach>
		</c:if>
      <hr>
      <a href="userNotification.jsp" class="w3-center">See All</a>
    </div>
  </li>
